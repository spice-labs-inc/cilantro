...
```
cilantro/<qualified suite or class>#<test name>     code tests
cilantro/<data path>[#<case id>]                    data-driven cases: the file's `id` field
```

Surveyor's `scripts/compare-test-ids.py` diffs those IDs against `tests/coverage/cilantro.tsv`
...
- Surveyor addresses corpus assemblies by `corpus/manifest.json` (`<id>@<version>/<tfm>`) and
  goldens by `corpus/golden-index.json`; keep those keys unique.
- What `spice` can observe of a .NET assembly is its MIME type and nuget purl (through
  goatrodeo); type, member and IL facts are cilantro-internal and stay unit-only.
- `src/cilantro/build.sbt` writes `META-INF/git/cilantro_3.properties` via a resource
  generator; the root `pom.xml` is only for publishing and does not build the jar.
